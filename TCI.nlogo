...
to plot-cluster-consensus
  ...
  [
    ...
    [
      ...
      set consensus concept-consensus sqrt (variance [ xcor ] of students with [ concept-no = i ])
      plotxy timer consensus
...
to-report concept-consensus [stdev]
  ;; This procedure is used to calculate the level and cluster consensus for a given
  ;; concept. The consensus is based on the standard deviation of the placement (position)
  ;; of each concept by each student. This method converts the standard deviation
  ;; into a value that ranges from 0% (least consensus) to 100% (most consensus).
  ;; To calculate this, a "max-stdev" is calculated based on the maximum distance
  ;; across the world view. Ideally, "variance [max-pcor min-pcor]" would be used
  ;; so that the world view can be re-sized; however, the variance procedure only
  ;; works with literal values, so max-pxcor and min-pycor must be "hard coded" in.
  let max-stdev sqrt (variance [15 -15])
...
The instructor can view the student concepts by clicking on a class concept. For example, if one of the class concepts shows some disagreement (e.g., it is orange or red), the instructor can click on the concept and see all of the student concepts associated with that concept to determine where the disagreement lies. A _View?_ selector is also provided that allows the instructor to view student concepts by simply hovering the mouse over the class concept. The _Cluster_ slider is used determine how the level of disagreement is represented:
